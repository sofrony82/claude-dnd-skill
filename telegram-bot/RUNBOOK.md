...
| Narration stops mid-sentence | the completion hit `max_tokens`, which counts reasoning too | `finish=length` in the journal. The default is no cap (`DND_DS_MAX_TOKENS=0`), but the provider still stops at 8192; the loop asks the model to continue up to `CONTINUE_LIMIT` times |
| `reply spent on reasoning, nothing said` | a completion used its whole budget reasoning (`finish=length`, `content=0`, `reasoning=8192`) — seen in heavy combat | reasoning is never sent back, so there is nothing to continue; the loop nudges instead, listing the dice already rolled this turn as final, so the retry narrates them rather than rolling again |
| The dice log under a turn has rolls the story never mentions | the DM rolled and then ignored or re-rolled | that is what the log is for; `grep "tool roll_dice"` the journal for the turn and compare with the narration. Tighten `COMBAT_RULES` in `prompts.py` if it recurs |
